...
(define (d.evaluate exp lex-env dyn-env)
   (if (atom? exp)
	  ...
	  ((lambda) (d.make-closure (cadr exp) (cddr exp) lex-env))
	  ;;((lambda-dynamic) (d.make-closure-dynamic (cadr exp) (cddr exp)))
	  (else (d.invoke (d.evaluate (car exp) lex-env dyn-env)
		   (evlis (cdr exp) lex-env dyn-env)
		   dyn-env)))))
...
;; lexical
(define (d.make-closure variables body lex-env)
   (lambda (vals dyn-env)
      (eprogn body (extend lex-env variables vals) dyn-env)))

;; ;; dynamic
;; (define (d.make-closure-dynamic variables body)
;;    (lambda (vals dyn-env)
;;       (eprogn body (extend dyn-env variables vals))))
...
(let ((v1 (d.evaluate '(let (a 1) a) env.init env.init))
      ...
      (v4 (d.evaluate '(let-dynamic (a 8) (let (f (lambda (x) (set-dynamic! a x))) (begin (f 9) (dynamic a)))) env.init env.init))
      )
